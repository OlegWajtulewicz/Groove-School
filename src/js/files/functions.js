	...
	function setTabsStatus(tabsBlock) {
		...
		function isTabsAnamate(tabsBlock) {
			if (tabsBlock.hasAttribute('data-tabs-animate')) {
				return tabsBlock.dataset.tabsAnimate > 0 ? Number(tabsBlock.dataset.tabsAnimate) : 1000;
			}
		}
			...
			tabsContent.forEach((tabsContentItem, index) => {
				if (tabsTitles[index].classList.contains('_tab-active')) {
					if (tabsBlockAnimate) {
						...
						tabsContentItem.classList.remove('hidden');
					}
					...
					if (tabsBlockAnimate) {
						...
						tabsContentItem.classList.add('hidden');
					}
				}
			});
		}
...
export function menuInit() {
	if (document.querySelector(".header__menu")) {
		document.addEventListener("click", function (e) {
			...
			} else if (e.target.closest('.col a'))  {
				menuClose();
			}
		});
	};
...
//================================================================================================================================================================================================================================================================================================================
//=================  Animate - tabs  ============================================================================================================ 
function observeHiddenClass(targetElement) {
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                // Проверяем если класс 'hidden' добавлен
    ...
    // Наблюдаем за изменениями классов у элемента
...
function setupIntersectionObserver() {
    ...
    // Выбор всех элементов с классом hidden
    const animElements = document.querySelectorAll(".hidden");
    animElements.forEach((animElement) => {
        observer.observe(animElement); // Добавляем отслеживание пересечения видимости
        observeHiddenClass(animElement); // Следим за изменениями класса 'hidden'
    });
}
